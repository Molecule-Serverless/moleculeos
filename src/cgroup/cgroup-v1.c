				     ...
				     bool threadgroup)
{
	...
	printk("[Dd@%s] Before cgroup_procs_write_start\n", __func__);
	task = cgroup_procs_write_start(buf, threadgroup);
	printk("[Dd@%s] After cgroup_procs_write_start\n", __func__);
	 ...
	 */
	printk("[Dd@%s] Before current_cred\n", __func__);
	cred = current_cred();
	printk("[Dd@%s] After current_cred\n", __func__);
	tcred = get_task_cred(task);
	printk("[Dd@%s] After get_task_cred\n", __func__);
	if (!uid_eq(cred->euid, GLOBAL_ROOT_UID) &&
	    !uid_eq(cred->euid, tcred->uid) &&
	    !uid_eq(cred->euid, tcred->suid))
		ret = -EACCES;
	put_cred(tcred);
	printk("[Dd@%s] After put_task_cred\n", __func__);
	if (ret)
		goto out_finish;

	ret = cgroup_attach_task(cgrp, task, threadgroup);
	printk("[Dd@%s] After cgroup_attack_task\n", __func__);
...
{
	printk("[Dd] Begin @ %s\n", __func__);
	ssize_t ret= __cgroup1_procs_write(of, buf, nbytes, off, true);
	printk("[Dd] End @ %s\n", __func__);
	return ret;
